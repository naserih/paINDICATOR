...
class Dicom_to_Imagestack:
    ...
    def make_array(self, PathDicom):
        ...
        if not self.get_images_mask:
            ...
            dicom_tag = [int(f.split(".")[-2]) for f in fileList if f.find('CT') == 0 ]
            dicom_files = [os.path.join(PathDicom, f) for f in fileList if f.find('CT') == 0 ]
            self.dicom_names = [x for _,x in sorted(zip(dicom_tag,dicom_files))]
            # print 'NNNNNNN ', self.dicom_names
            self.reader.SetFileNames(self.dicom_names)
            self.get_images()
            image_files = [i.split(PathDicom)[1][1:] for i in self.dicom_names]
            RT_Files = [os.path.join(PathDicom, f) for f in fileList if f.find('RT') == 0 or f.find('RS') == 0]
    ...
    def get_mask(self):
        ...
        # print '>>ssssssssssssssssssssssssss>>>   ', self.RS_struct.dir()
